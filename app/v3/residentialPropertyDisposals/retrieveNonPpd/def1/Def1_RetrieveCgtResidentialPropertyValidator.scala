...
@Singleton
class Def1_RetrieveCgtResidentialPropertyValidator @Inject() (nino: String, taxYear: String)
  ...
  override def validate: Validated[Seq[MtdError], RetrieveCgtResidentialPropertyRequestData] =
    ResolveNino(nino).map(validNino => Def1_RetrieveCgtResidentialPropertyRequestData(validNino, TaxYear.fromMtd(taxYear)))

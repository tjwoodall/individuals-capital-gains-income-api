...
class Def1_RetrieveCgtPpdOverridesValidatorSpec extends UnitSpec with MockAppConfig {
  ...
  "validator" should {
    "return the parsed domain object" when {
      "a valid request is supplied" in new Test {
        ...
        result shouldBe Right(Def1_RetrieveCgtPpdOverridesRequestData(parsedNino, parsedTaxYear, parsedSource))
      }
      ...
      "a valid request is supplied with no source" in new Test {
        ...
        result shouldBe Right(Def1_RetrieveCgtPpdOverridesRequestData(parsedNino, parsedTaxYear, MtdSourceEnum.latest))
      }
    }
